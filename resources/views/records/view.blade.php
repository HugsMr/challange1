...
<div class="card">
  ...
  <div class="card-body">
      <form action="{{ url('/api/records/' .$records->id) }}" method="post">
        ...
        <input type="text" name="title" id="title" value="{{$records->title}}" class="form-control" required="required" readonly></br>
        <label>Content</label></br>
        <textarea rows="5" cols="100" name="recordContent" required="required" readonly>{{$records->recordContent}}</textarea>
        <br>
